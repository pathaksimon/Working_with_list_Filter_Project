...
class App extends Component {
  ...
  lendu = () => {
    ...
    return filterdProjects.map(projectDetails => (
      <ProjectItem
  ...
  changeinstyle = tabDetails => {
    const {activeTabId} = this.state
    const charan = tabDetails.tabId === activeTabId
    return charan
  }
        ...
        <ul className="tabs-container">
          {tabsList.map(tabDetails => (
            <TabItem
              ...
              changeinstyle={this.changeinstyle(tabDetails)}
            />
          ))}
        </ul>

        <ul className="project-list-container">{this.lendu()}</ul>
      </div>
    )
  }
}
